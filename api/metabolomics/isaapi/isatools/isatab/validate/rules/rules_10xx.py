...
from metabolomics.isaapi.isatools.utils import utf8_text_file_open
from metabolomics.isaapi.isatools.isatab.load import load_table
from metabolomics.isaapi.isatools.isatab.defaults import _RX_FACTOR_VALUE, _RX_PARAMETER_VALUE, log
from metabolomics.isaapi.isatools.isatab.validate.store import validator
from metabolomics.isaapi.isatools.isatab.utils import cell_has_value
...
def check_protocol_usage(i_df, dir_context):
    ...
    for i, study_df in enumerate(i_df['studies']):
        ...
        #20230714 converted protocols found in assay files to uppercase 
        # to take care of case misamatch in the investigation file and study files
        study_protocols = i_df['s_protocols'][i]['Study Protocol Name'].tolist() 

        #protocols_declared = set(i_df['s_protocols'][i]['Study Protocol Name'].tolist())

        protocols_declared = set(protocol.upper() for protocol in study_protocols)
        #print("Proto used", study_protocols)
        ...
        if study_filename != '':
            try:
                ...
                with utf8_text_file_open(path.join(dir_context, study_filename)) as s_fp:
                    ...
                    protocol_refs_used = set([r.upper() for r in protocol_refs_used if notnull(r)])

                    #print("******Proto refs used", protocol_refs_used)
