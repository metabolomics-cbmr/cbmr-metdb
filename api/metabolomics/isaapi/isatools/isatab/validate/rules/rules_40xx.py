...
from metabolomics.isaapi.isatools.io import isatab_configurator
from metabolomics.isaapi.isatools.isatab.validate.store import validator
from metabolomics.isaapi.isatools.isatab.defaults import (
    log,
    _RX_INDEXED_COL,
    _RX_CHARACTERISTICS,
    _RX_PARAMETER_VALUE,
...
def load_table_checks(df, filename):
    ...
    for x, column in enumerate(columns):  # check if columns have valid labels
        ...
        #20230713 added 'Derived Data File' to the list of recognised column names 
        #so that 'a_...' data from Uni team need not be manually modified
        ...
        if (column not in ['Source Name', 'Sample Name', 'Term Source REF',
                           ...
                           'Acquisition Parameter Data File', 'Derived Data File']) \
    ...
    for object_columns in object_columns_list:
        ...
        if prop_name in ['Sample Name', 'Source Name']:
            ...
            for x, col in enumerate(object_columns[2:]):
                if col not in ['Term Source REF', 'Term Accession Number']:
                    log.error(
                        "(E) Unexpected column 2 heading following {} column. "
