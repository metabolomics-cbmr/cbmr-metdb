from sqlalchemy import Column, String, ForeignKey, Integer, select 
from sqlalchemy.orm import relationship


from metabolomics.isaapi.isatools.model import OntologyAnnotation as OntologyAnnotationModel
from metabolomics.isaapi.isatools.database.models.relationships import (
...
def make_ontology_annotation_methods() -> None:
    ...
    def to_sql(self, session):
        ...
        stmt = select(OntologyAnnotation).where(OntologyAnnotation.annotation_value == self.term  \
                 and OntologyAnnotation.term_source_id  == self.term_source_id  \
                                                if OntologyAnnotation.term_source_id else None)                                               
        ...
        for row in result:
            oa_id = row.OntologyAnnotation.ontology_annotation_id 
            break

        oa = session.query(OntologyAnnotation).get(oa_id)
        #print("In ontology annotation 1", self.term)
        if oa:
            return oa
        ...
        session.add(oa)
        session.commit()
        #print("In ontology annotation 2")
