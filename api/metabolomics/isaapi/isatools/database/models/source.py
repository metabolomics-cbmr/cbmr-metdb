...
from metabolomics.isaapi.isatools.model import Source as SourceModel
from metabolomics.isaapi.isatools.database.models.relationships import study_sources, source_characteristics, sample_derives_from
from metabolomics.isaapi.isatools.database.models.inputs_outputs import InputOutput
...
def make_source_methods():
    ...
    def to_sql(self, session: Session) -> Source:
        ...
        #source = session.query(Source).get(self.name)
        
        #print("Source 1", self.name)
        if source:
            return source

        #print("In source 2", self.name)

        #20230725  source inserted into database as soon as it is encountered
        #this was done to remove duplicate row error for source table 
        #added session.add(srcp),session(commit()
        srcp =  Source(
            source_id=self.id,
            name=self.name,
