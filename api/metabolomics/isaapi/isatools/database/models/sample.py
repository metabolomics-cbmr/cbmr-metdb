...
from metabolomics.isaapi.isatools.model import Sample as SampleModel
...
def make_sample_methods():
    ...
    def to_sql(self, session: Session) -> Sample:
        ...
        #print('In sample 1', self.name)                

        if sample:
            #print('In sample 1a - ', sample)                            
            return sample

        #print('In sample 2')                
        #20230725  sample  inserted into database as soon as it is encountered
        #this was done to remove duplicate row error for sample table 
        #added session.add(samp),session(commit()
        samp =  Sample(
            sample_id=self.id,
            name=self.name,
